...
class SysTrayIcon(object):
    ...
    def __init__(self, icon, hover_text, menu_options, popup_title, popup_msg, on_quit=None, default_menu_index=None, window_class_name=None,):
    ...
    def set_popup(self):
        hicon = win32gui.LoadIcon(0, win32con.IDI_APPLICATION)
        win32gui.Shell_NotifyIcon(win32gui.NIM_MODIFY, (self.hwnd, 0, win32gui.NIF_INFO, win32con.WM_USER+20, hicon, "Balloon tooltip",self.popup_msg,200,self.popup_title))
...
def run () :
    ...
    def icon_popup(sysTrayIcon):
        sysTrayIcon.set_popup()

    def bye(sysTrayIcon) :
        print('Bye, then.')
        os.system("taskkill /im webapp.exe")
        raise SystemExit
        sys.exit(0)
    ...
    menu_options = (
        ...
        ('Show Popup', None, icon_popup),
        ('A sub-menu', next(icons),
            (('Say Hello to Simon', next(icons), simon), ('Switch Icon', next(icons), switch_icon),)
         )
    )

    tray = SysTrayIcon(next(icons), hover_text, menu_options, 'CloudLink', 'Hello Brian :)', on_quit=bye, default_menu_index=1)
